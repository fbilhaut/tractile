...
pub(super) fn load_model(config: &Configuration) -> crate::Result<Model> {
    ...
    cfg_select! {
        ...
        feature = "cuda" => {
            load_cuda_model(inference_model)
        }
        _ => {
...
#[cfg(feature = "cuda")]
fn load_cuda_model(inference_model: InferenceModel) -> crate::Result<Model> {
    use tract_core::transform::ModelTransform;
    ...
    let typed = inference_model.into_typed()?;
    let mut m = CudaTransform::default().transform_into(typed)?;
